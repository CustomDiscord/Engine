...
class changelog extends Plugin {
  ...
  displayChangelog() {
    ...
    for (const version of keys) {
      output.push(
        `<h1 className='added-3Q70Gu title-1PW5Fd marginTop-4_cfcL marginTop20-3UscxH'${
      ...
      for (const key in changelogData[version]) {
        output.push(`
          <h5 className='titleDefault-1CWM9y title-3i-5G_ marginReset-3hwON1 marginTop20-3UscxH weightMedium-13x9Y8 size16-3IvaX_ height24-2pMcnc flexChild-1KGW5q'>
            ${key}
          </h5>`);
    ...
    this.CD.plugins.get('react')
      .createModal(`<div class="flex-lFgbSz flex-3B1Tl4 horizontal-2BEEBe horizontal-2VE-Fw flex-3B1Tl4 directionRow-yNbSvJ justifyStart-2yIZo0 alignCenter-3VxkQP noWrap-v6g9vO header-3sp3cE">
        <div className='flexChild-1KGW5q' style='flex: 1 1 auto;'>
          <h4 className='h4-2IXpeI title-1pmpPr size16-3IvaX_ height20-165WbF weightSemiBold-T8sxWH defaultColor-v22dK1 defaultMarginh4-jAopYe marginReset-3hwONl'>
            CustomDiscord Changelog
          </h4>
          <div className='guildName-1u0hy7 small-3-03j1 size12-1IGJ19 height16-1qXrGy primary-2giqSn'>Current Version ${
            this.CD.verison
          }</div>
        ...
        <div className='scrollerWrap-2uBjct content-1Cut5s scrollerThemed-19vinI themeGhostHairline-2H8SiW'>
          <div className='scroller-fzNley inner-tqJwAU content-3KEfmo'>
            ${output.join('\n')}
          </div>
        </div>
      </div>`);
